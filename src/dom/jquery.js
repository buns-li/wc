const time_now = Date.now || function() {
...
wc
    ...
    .inject('res', {
        fetch: function(url) {
            let def = jQ.Deferred()
            loadJS_CSS(url, (err, data) => err ? def.reject(err) : def.resolve('ok'))
            return def.promise()
        }
    }, true)
...
let $w = jQ(window)
    .on('scroll', throttle(() => wc.pub('scroll', [$w.scrollTop(), $w.scrollLeft()]), 300))
    .on('resize', throttle(() => wc.pub('resize', [$w.width(), $w.height(), $doc.width(), $doc.height()]), 300))
    .on('loaded', () => wc.pub('loaded'))