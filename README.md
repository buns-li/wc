...
* `url`: [`Function`] 获取资源文件地址的函数(*Optional*)
...
通过wc.define(id,options,factory)的api调用形式

#### 组件配置参数

* `id`: [`String`] 组件的唯一标识(**Required**)
* `options`: [`Object-KV`] 组件的对外选项定义(**Optional**)
...
3. 暂停/恢复模式
  ...
  * pause() 和 resume() 操作必须成对出现,否则堆积一多就会造成内存增长
  ...
  wc.define('cmp2',{},function(){
      var self = this
      self.on('recieve_e1',function(data){
            var scope = this
